...
**SBR (sidebar right) items:** RIT value · MIC gain · DSP level · LEN (filter)  
...
2. `scan_tx_on()` → CLK2 (QSE) at step frequency, T/R switch asserted.  
3. Wait 30 ms settle, call `Analog_Update()` twice.  
4. Record `g_analog.swr_x100`.  
5. `scan_tx_off()` → T/R switch released, CLK2 disabled, QSD restored to centre frequency.  
...
### Si5351A – 4× LO with external 74LVC74 quadrature divider
...
Si5351A
    ...
    │     └─ CLK0  →  4 × RF frequency  (RX LO)
    │                      │
    │               74LVC74 dual D flip-flop
    │               ÷4 Johnson counter
    │                  ├─ I  (0°)   → QSD mixer I port
    │                  └─ Q (90°)  → QSD mixer Q port
    │
    └─ PLL_B  VCO 600–900 MHz
          └─ CLK2  →  4 × TX frequency  (TX LO)
                           │
                    74LVC74 (same device or second)
                       ├─ I  → QSE mixer I port
                       └─ Q  → QSE mixer Q port
```

CLK1 is powered down and unused.

**Quadrature method:** The 74LVC74 dual D flip-flop is wired as a Johnson (divide-by-4) counter. This produces two outputs with a precise 90° phase relationship from a single 4× clock. No Si5351 phase-offset registers are used for quadrature; this avoids the even-MS\_div constraint and gives better I/Q balance than the register method.

**Programming API:**

```c
SI5351_SetQSDFrequency(&g_si5351, rf_hz);   // CLK0 = rf_hz × 4, PLL_A
SI5351_SetQSEFrequency(&g_si5351, tx_hz);   // CLK2 = tx_hz × 4, PLL_B
```

Callers always pass the RF frequency; the ×4 factor is inside the driver.

**LO offset (`lo_offset_hz`, default 18 kHz):** The Si5351 is programmed to `freq_hz + lo_offset_hz` for RX. The NCO then digitally shifts baseband by the same offset in the opposite direction, placing the desired signal exactly at DC after mixing. This keeps the QSD operating at a slight offset from the tuned frequency, which avoids self-reception of the LO at the antenna.
...
**Constraints:**
- VCO range: 600–900 MHz (PLL_A and PLL_B independently).
- CLK0 output range: ~100 kHz to ~150 MHz (limited by MS integer divider 6–2047 + R divider 1..128).
- XTAL: 25 MHz on-board (runtime value `g_sdr.lo_offset_hz`; `SI5351_XTAL_FREQ_HZ` in `si5351.h` is 27 MHz but is overridden by `SI5351_XTAL_HZ = 25000000` in `csdr_app.h` at init time).
- PLL_A and PLL_B are independent: RX and TX LOs can be programmed simultaneously without interference.
- PLL reset is issued after VCO reprogramming to guarantee phase coherence.
