...
/// Incremental variant of ftx_find_candidates() for cooperative (sliced) scheduling on MCUs:
/// scores only time offsets in [time_min, time_max) and accumulates candidates into the
/// caller-owned min-heap.  Initialise *heap_size_io to 0 before the first slice, keep passing
/// the same heap/heap_size_io for subsequent slices (full range equivalent: -10..20), then
/// call ftx_candidates_sort() once after the last slice to obtain a descending-score array.
int ftx_find_candidates_range(const ftx_waterfall_t* power, int num_candidates, ftx_candidate_t heap[], int min_score,
