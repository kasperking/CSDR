/**
  ...
  *
  *    SI5351 CLK2 ──► PA8  (TIM1_CH1, AF1)  → external clock mode 1 (TI1FP1)
  *    GPS 1PPS    ──► PA9  (TIM1_CH2, AF1)  → input capture (latch phần cứng)
  *
  ...
  *
  *  Cả hai ISR đều chỉ vài chục cycle, priority 12 — không ảnh hưởng
  *  audio/USB pipeline (priority 0).  Ngoài phiên đo TIM1 tắt hoàn toàn.
  ******************************************************************************
  */
#ifndef __GPS_CAL_H
#define __GPS_CAL_H
...
typedef enum {
  ...
  GPSCAL_NO_CLK,       /*!< không thấy CLK2 vào PA8 (kiểm tra dây/CLK2) */
...
/** Bật TIM1 + GPIO và bắt đầu đo.  nominal_hz = tần số danh định của
 *  tín hiệu đưa vào PA8 (= g_si5351.xtal_hz khi dùng CLK2 passthrough). */
void GPSCal_Start(uint32_t nominal_hz);

/** Dừng đo, tắt TIM1/IRQ, trả PA8/PA9 về trạng thái mặc định. */
