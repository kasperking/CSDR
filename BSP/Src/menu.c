...
void Menu_Init(Menu_Handle_t *m)
{
  ...
  m->items[10] = (MenuItem_t){ "RIT(Hz)", MENU_TYPE_INT, -999,999,1,  &_rit_val,   NULL,      0U, NULL,NULL,0 };
  m->items[11] = (MenuItem_t){ "RX Shift",MENU_TYPE_INT,-2000,2000,50,&_rxshift_val,NULL,     0U, NULL,"Hz",0 };
  m->items[12] = (MenuItem_t){ "Notch",   MENU_TYPE_ENUM, 0,0,0,      &_notch_val, onoff_strs,2U, NULL,NULL,0 };
  m->items[13] = (MenuItem_t){ "Notch Hz",MENU_TYPE_INT,  100,4000,50,&_notchhz_val,NULL,     0U, NULL,"Hz",0 };
  m->items[14] = (MenuItem_t){ "Span",    MENU_TYPE_ENUM, 0,0,0,      &_zoom_val,  zoom_strs, 4U, NULL,NULL,0 };

  /* ── Audio group (parent = 1) ───────────────────────────── */
  m->items[15] = (MenuItem_t){ "Volume",    MENU_TYPE_INT, 0,100,5, &_vol_val, NULL,0U,NULL,NULL,1 };
  m->items[16] = (MenuItem_t){ "Mic Gain",  MENU_TYPE_INT, 0,100,1, &_mic_val, NULL,0U,NULL,NULL,1 };
  m->items[17] = (MenuItem_t){ "Digi Drive",MENU_TYPE_INT, 0,100,1, &_digi_val,NULL,0U,NULL,NULL,1 };
  ...
  m->items[22] = (MenuItem_t){ "Ext ALC",  MENU_TYPE_ENUM, 0,0,0,      &_alc_val,     onoff_strs,2U,NULL,NULL,3 };
  m->items[23] = (MenuItem_t){ "TX Low",   MENU_TYPE_INT, 100,500,50,  &_tx_low_val,  NULL,      0U,NULL,"Hz",3 };
  m->items[24] = (MenuItem_t){ "TX High",  MENU_TYPE_INT,2200,3500,100,&_tx_high_val, NULL,      0U,NULL,"Hz",3 };
