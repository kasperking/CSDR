...
// Incremental variant of the candidate search: scores only time offsets in
// [time_min, time_max) and accumulates results into the caller's min-heap.
// Lets a cooperative scheduler split the full search (~tens of ms) into
// ~1-2 ms slices (one or two time offsets per call) so the main-loop audio
// pump is never starved.  Call ftx_candidates_sort() once after the last slice.
int ftx_find_candidates_range(const ftx_waterfall_t* wf, int num_candidates, ftx_candidate_t heap[], int min_score,
                              int time_min, int time_max, int* heap_size_io)
{
    ...
    for (candidate.time_sub = 0; candidate.time_sub < wf->time_osr; ++candidate.time_sub)
    {
        ...
        {
            for (candidate.time_offset = time_min; candidate.time_offset < time_max; ++candidate.time_offset)
            {
...
{
    int heap_size = 0;
    ftx_find_candidates_range(wf, num_candidates, heap, min_score, -10, 20, &heap_size);
    ftx_candidates_sort(heap, heap_size);
    return heap_size;
}

void ftx_candidates_sort(ftx_candidate_t heap[], int heap_size)
{
